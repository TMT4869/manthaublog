...
public interface PostRepository extends JpaRepository<Post, UUID> {
    ...
    @Query("SELECT p FROM Post p WHERE p.status = :status AND (:language IS NULL OR p.language = :language) AND (:authorId IS NULL OR p.authorId = :authorId)")
    Page<Post> findPublished(PostStatus status, String language, UUID authorId, Pageable pageable);
